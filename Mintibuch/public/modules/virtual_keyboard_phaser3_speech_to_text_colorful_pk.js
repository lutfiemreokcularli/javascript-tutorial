...
if (SpeechRecognition) {
  ...
  recognition.lang = "de-DE"; // Türkçe için
  ...
  recognition.onresult = (event) => {
    ...
    inputString += transcript;
    updateInputDisplay(neonPipeline.scene);

    // 🎯 Tekrar başlat (kapanma ihtimaline karşı)
    if (micActive) {
      recognition.stop(); // önce durdur
      setTimeout(() => {
        try {
          recognition.start(); // biraz bekledikten sonra başlat
        } catch (e) {
          console.warn("Mikrofon zaten çalışıyor olabilir:", e.message);
        }
      }, 250);
    }
  };
  ...
  /* recognition.onend = () => {
    debugger;
    if (micActive) {
      recognition.start(); // 🔁 Mikrofon açıksa tekrar başlat
    }
  }; */

  recognition.onerror = (event) => {
    console.warn("Sesli giriş hatası:", event.error);
  };
} else {
...
function create() {
  ...
  const micBtn = this.add
    .rectangle(this.scale.width - 200, 40, 100, 40, 0x000000, 0)
    .setInteractive({ cursor: "pointer" })
    .setPipeline("neon-border");

  const micLabel = this.add
    .text(micBtn.x, micBtn.y, "🎤 MIC", {
      font: "18px Arial",
      color: "#39ff14",
    })
    .setOrigin(0.5);

  micBtn.on("pointerdown", () => {
    if (!recognition) return;

    if (!micActive) {
      recognition.start();
      micLabel.setText("🛑 STOP");
      micActive = true;
    } else {
      recognition.stop();
  ...
  this.input.keyboard.on("keydown", (event) => {
    if (event.key === "Backspace") {
      ...
      // Shift açıkken büyük, değilse küçük karakter
      const char = shift ? event.key.toUpperCase() : event.key.toLowerCase();
      inputString += char;
