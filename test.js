$(document).ready(function () {
    ...
    Comics.PlayScene.prototype = {
        ...
        create: function () {
            if (this.pageInfoText) this.pageInfoText.setText((this.counter + 1) + ' / ' + myPages.data.length);
            ...
            for (var i = 0, dat; i < this.pageInfo.length; i++) {
                ...
                if (typeof dat === 'object') {
                    var elem = this.add[dat.type](parseFloat(dat.place[0]), parseFloat(dat.place[1]), dat.img);
                    elem.anchor.set(.5, 1);
                    if (dat.type === 'button') {
                        ...
                        if (!Comics.config.debug) {
                            ...
                            elem.events.onInputOver.add(Minti.PhaserHelper.onInput.Over, this);
                        }
                        Minti.PhaserHelper.getText(this, parseFloat(dat.place[0]), parseFloat(dat.place[1]) + parseFloat(myPages.textPlace) + (parseFloat(dat.textPlace || 0)), .5, dat.text || '', { font: '35px Amaranth' }, 'part');
                    }
                    if (Comics.config.debug) {
        ...
        addNavigation: function () {
            if (this.counter != myPages.data.length - 1) {
                this.next = this.add.button(this.world.width - 70, 550, 'next', function () {
            ...
            this.pageInfoText = Minti.PhaserHelper.getText(this, this.world.centerX, 1030, .5, (this.counter + 1) + ' / ' + myPages.data.length, {
                font: "bold 40px Amaranth",
                fill: "#f5faf6",
            }, 'world');
            if (this.counter != 0) {
                this.prev = this.add.button(70, this.next.y, 'prev', function () {
                    sound = this.add.sound("page", 0.2, false);
                    sound.play();
                    this.counter--;
                    if (this.counter == 0) {
    ...
    core.state.add('Preload', new Minti.PhaserHelper.PreloadState('PlayScene', Comics.Assets, core));
